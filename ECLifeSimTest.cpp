...
void Test1()
{
  ...
  sim.AddDailyEvent(1.0, indDodo, 1, 0);
  sim.AddDailyEvent(2.0, indDodo, 1, 0);
  sim.AddDailyEvent(9.0, indDodo, 1, 0);
  //sim.AddDailyEvent(12.0, indDodo, 0, 0);
  double tmStart = 0.0;
    const double tmLimit = 24.0;
...
void Test2()
{
    ...
    sim.AddDailyEvent(4.0, indYoyo, 1, 1);
...
void Test5()
{
    ...
    ECGrasshopper grassy("grassy", &qoqo);
    int indGrassy = sim.AddOrganism(&grassy);
    // grassy just attaks qoqo once a day
    sim.AddDailyEvent(5.0, indGrassy, 1, 1);
...
void Test7()
{
    ...
    double dur = sim.Simulate(tmStart, tmLimit);
    cout << "Duration of simulation: " << dur << endl;
    cout << "Vitality of Popo: " << sim.GetVitalityFor("popo") << endl;
}

void DeadGrasshopperTwoSpiders() {
  ECLifeSimulator sim;
  
  ECGrasshopper hoho("hoho");
  int idHoho = sim.AddOrganism(&hoho);
  
  ECSpider popo("popo", &hoho);
  int idPopo = sim.AddOrganism(&popo);
  sim.AddDailyEvent(1.0, idPopo, 1,1);
  
  ECSpider coco("coco", &hoho);
  int idCoco = sim.AddOrganism(&coco);
  sim.AddDailyEvent(3.0, idCoco, 1, 1);

  double start = 0.0;
  double limit = 30.0;
  double dur = sim.Simulate(start, limit);
  // 30
...
void MaxEatingTimes() {
  ...
  ECSpider popo("popo");
  int indPopo = sim.AddOrganism(&popo);
  sim.AddDailyEvent(1.0, indPopo, 0, 0);
  sim.AddDailyEvent(1.0, indPopo, 1, 0);
  sim.AddDailyEvent(2.0, indPopo, 0, 0);
  sim.AddDailyEvent(3.0, indPopo, 0, 0);
  sim.AddDailyEvent(20.0, indPopo, 0, 0);
